...
2. Scaffold under `services/<service-name>/` as its own crate, in **Clean Architecture**
   ...
   - `src/domain/` — entities (plain structs) and domain errors (e.g. `enum BookingError {
     SeatUnavailable, .. }`), plus the **port traits** this service needs (`trait
     SeatRepository`, and any outbound gateway like `trait PaymentGateway`), using
     `#[async_trait]` (crate `async-trait`) so they're usable as `Arc<dyn Trait + Send +
     Sync>`. Zero imports of `sqlx`/`axum`/`tokio` here. Business invariants live as methods on
     the entity (e.g. `Seat::reserve(&mut self) -> Result<(), BookingError>`) — the entity
     enforces the rule, not the caller.
   - `src/usecase/` — one struct per use case (e.g. `BookSeatUseCase`), holding `Arc<dyn
     SeatRepository>` (and other ports) injected via `new()`. Imports `domain` only, never
     `adapter`, never `sqlx`/`axum` directly. Multi-step business flows are orchestrated here
     inside a single repository-driven transaction call — see `/review-concurrency` for why
     that matters for booking-service specifically.
   ...
   - `src/adapter/repository/postgres.rs` — implements the `domain::SeatRepository` trait using
     `sqlx::PgPool`. This is the only module allowed to import `sqlx` — the DB driver is a
     detail, not something `usecase`/`domain` should know exists.
   - `src/platform/` — cross-cutting infrastructure with no business meaning: DB pool
     construction, tracing/logging setup, config loading. Used only from `main.rs`.
   - `src/main.rs` — the **composition root**: the only file that wires every layer together
     (`let repo: Arc<dyn SeatRepository> = Arc::new(PostgresSeatRepository::new(pool));`,
     `let uc = BookSeatUseCase::new(repo);`, then builds the `axum::Router` with `uc` in its
     state) plus the process lifecycle (below). No business logic here, only construction,
...
3. Wire in these scalability/stability patterns, placed in the layer that owns them — do not
   ...
   - **DB connection pool**: built once in `src/platform/db.rs` via
     ...
     `main.rs`, passed into the postgres adapter; `usecase`/`domain` never see `PgPool`
     directly.
...
5. Confirm the route prefix implemented in the router matches `kong.yml` exactly. Also verify
   ...
   modules, `usecase` only `use`s `domain`, `adapter::http` and `adapter::repository` don't
   depend on each other — grep for cross-module `use` statements if unsure. Then tell the user
